...
def pandas_candlestick_ohlc(dat, stick="day", otherseries=None, size=(12, 8)):
    """
    ...
    transdat = dat[["High", "Low", "Open", "Close"]]
    if (type(stick) == str):
        if stick == "day":
            plotdat = transdat
            ...
            if stick == "week":
                ...
                transdat["month"] = pd.to_datetime(
                    transdat.index).map(lambda x: x.month)  # Identify months
        ...
        raise ValueError('Valid inputs to argument "stick" include the strings "day",\
